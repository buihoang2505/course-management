    ...
    <div>
      <div class="page">
        ...
        <div class="admin-shell">
          <div :class="['sidebar', {'sidebar-open': sidebarOpen}]" @click.self="sidebarOpen=false">
            <button v-for="t in tabs" :key="t.key"
                    :class="['stab', { active: activeTab===t.key }]"
                    @click="switchTab(t.key); sidebarOpen=false">
          ...
          <div class="panel">
            <div class="panel-hd">
              ...
              <template v-else-if="activeTab==='applications'">
                <span class="phd-title">📝 Đơn đăng ký Giảng viên</span>
                <div class="hd-right">
                  <select v-model="appFilter" @change="loadApps" class="sel">
              ...
              <template v-else-if="activeTab==='qa'">
                ...
                <div class="hd-right">
                  <button :class="['ab', !qaAdminFilter?'ab-active':'']"
                          @click="qaAdminFilter=false;loadAdminQA()">⏳ Chưa xử lý</button>
                  <button :class="['ab', qaAdminFilter?'ab-active':'']"
                          @click="qaAdminFilter=true;loadAdminQA()">✅ Đã xử lý</button>
                </div>
              </template>
              <template v-else-if="activeTab==='payments'">
                <span class="phd-title">💰 Thanh toán</span>
              </template>
            </div>

            <!-- panel-body chứa TẤT CẢ các tab -->
            <div class="panel-body">
              ...
              <div v-show="activeTab==='users'" class="tab-inner">
                ...
                <div v-if="totalUserPages>1" class="tbl-pagination">
                  <button class="pg-btn" :disabled="userPage===1" @click="userPage--">‹</button>
                  <button v-for="p in pgNums(totalUserPages,userPage)" :key="p"
                          :class="['pg-btn',{active:p===userPage,ellipsis:p==='...'}]"
                          :disabled="p==='...'" @click="p!=='...'&&(userPage=p)">{{p}}</button>
                  <button class="pg-btn" :disabled="userPage===totalUserPages" @click="userPage++">›</button>
                  <span class="pg-info">{{ filteredUsers.length }} học viên</span>
                </div>
              </div>
              ...
              <div v-show="activeTab==='grades'" class="tab-inner">
                ...
                <template v-else>
                  <div class="grade-info-box">
                    ...
                    <span>Điểm tự động = <strong>TB điểm cao nhất của các quiz</strong> (thang 10). Khóa không có quiz sẽ không có điểm tự động.</span>
                  </div>
                  ...
                  <table class="tbl">
                    <thead><tr><th>#</th><th>Học viên</th><th>Tiến độ</th><th>Quiz đã làm</th><th>Điểm quiz TB</th><th>Thao tác</th></tr></thead>
                    <tbody>
              ...
              <div v-show="activeTab==='certificates'" class="tab-inner">
                ...
                <table v-else class="tbl">
                  <thead>
                  <tr>
                    <th>ID</th><th>Học viên</th><th>Khóa học</th><th>Điểm TB Quiz</th><th>Ngày cấp</th><th>Mã chứng chỉ</th><th>Thao tác</th>
                  </tr>
                  </thead>
                  <tbody>
                  ...
                  <tr v-for="c in filteredCerts" :key="c.id">
                    ...
                    <td>
                      <span :class="['schip', sc(c.averageQuizScore/10)]">{{ c.averageQuizScore }}%</span>
                    </td>
                    <td class="muted-col">{{ formatDate(c.issuedAt) }}</td>
                    <td>
                      <a :href="`/verify/${c.certificateCode}`" target="_blank" class="cert-code-link" :title="c.certificateCode">
              ...
              <div v-show="activeTab==='notifications'" class="tab-inner">
                ...
                <div v-else class="notif-scroll-wrap">
              ...
              <!-- APPLICATIONS -->
              <div v-show="activeTab==='applications'" class="tab-inner">
                <div class="app-filter-bar">
                  <button :class="['ab','ab-filter',!appFilter?'ab-filter-active':'']" @click="appFilter='';loadApps()">Tất cả</button>
                  <button :class="['ab','ab-filter','ab-yellow',appFilter==='PENDING'?'ab-filter-active':'']" @click="appFilter='PENDING';loadApps()">⏳ Chờ</button>
                  <button :class="['ab','ab-filter','ab-green2',appFilter==='APPROVED'?'ab-filter-active':'']" @click="appFilter='APPROVED';loadApps()">✅ Duyệt</button>
                  <button :class="['ab','ab-filter','ab-red2',appFilter==='REJECTED'?'ab-filter-active':'']" @click="appFilter='REJECTED';loadApps()">❌ Từ chối</button>
                  <button @click="loadApps()" class="btn btn-ghost btn-sm" style="margin-left:auto">↺</button>
                </div>
                <div v-if="loadingApps" class="center-state"><div class="spinner"></div></div>
                <div v-else-if="!apps.length" class="center-state">
                  <div class="empty-ico">📝</div>
                  <p>{{ appFilter ? 'Không có đơn nào' : 'Chưa có đơn đăng ký Giảng viên' }}</p>
                </div>
                <div v-else class="app-cards-wrap">
                  <div class="app-card" v-for="a in apps" :key="a.id">
                    <div class="app-card-head">
                      <div class="ucell">
                        <div class="uav" :style="a.avatar?`background:url(${a.avatar}) center/cover`:''">{{ a.avatar?'':(a.username||'?')[0].toUpperCase() }}</div>
                        <div>
                          <div class="uname">{{ a.fullName || a.username }}</div>
                          <div class="uid">{{ a.email }}</div>
                        </div>
                      </div>
                    ...
                    <div class="app-card-body">
                      <div class="app-field"><span class="app-label">Kinh nghiệm</span><span>{{ a.experience }}</span></div>
                      <div class="app-field"><span class="app-label">Chuyên môn</span><span class="badge badge-blue" v-if="a.expertise">{{ a.expertise }}</span><span v-else class="muted">—</span></div>
                      <div class="app-field" v-if="a.portfolioUrl"><span class="app-label">Portfolio</span><a :href="a.portfolioUrl.startsWith('http')?a.portfolioUrl:'https://'+a.portfolioUrl" target="_blank" class="app-link">🔗 Xem</a></div>
                      <div class="app-field"><span class="app-label">Ngày nộp</span><span style="font-size:.78rem">{{ new Date(a.createdAt).toLocaleDateString('vi-VN') }}</span></div>
                      <div class="app-field" v-if="a.adminNote"><span class="app-label">Ghi chú</span><span style="font-size:.78rem;font-style:italic;color:var(--muted)">💬 {{ a.adminNote }}</span></div>
                    </div>
                    <div v-if="a.status==='PENDING'" class="app-card-actions">
                      <button @click="approveApp(a)" class="ab ab-green" style="flex:1">✅ Duyệt</button>
                      <button @click="openReject(a)" class="ab ab-red" style="flex:1">❌ Từ chối</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div v-show="activeTab==='qa'" class="tab-inner">
                <div v-if="loadingAdminQA" class="center-state"><div class="spinner"></div></div>
                <div v-else-if="!adminQAList.length" class="center-state">
                  <div class="empty-ico">💬</div>
                  <p>{{ qaAdminFilter ? 'Không có câu hỏi đã xử lý' : 'Không có câu hỏi nào đang chờ 🎉' }}</p>
                </div>
                <div v-else class="qa-inbox">
                  <div v-for="q in adminQAList" :key="q.id" class="qi-card" :class="{resolved:q.resolved}">
                    <div class="qi-head">
                      <div class="uav" style="width:26px;height:26px;font-size:.68rem;flex-shrink:0">
                        {{ (q.author||'?')[0].toUpperCase() }}
                      </div>
                      <div class="qi-meta">
                        <span class="qi-author">{{ q.author }}</span>
                        <span class="qi-context">📖 {{ q.lessonTitle }} · 📚 {{ q.courseTitle }}</span>
                        <span class="qi-time">{{ new Date(q.createdAt).toLocaleString('vi-VN') }}</span>
                      </div>
                      <div class="qi-acts">
                        <span v-if="q.resolved" class="badge badge-green" style="font-size:.66rem">✅ Đã xử lý</span>
                        <button @click="toggleAdminQAResolve(q)" class="ab" style="font-size:.72rem">
                          {{ q.resolved?'↩':'✓' }}
                        </button>
                        <button @click="delAdminQA(q.id)" class="ab ab-red" style="font-size:.72rem">🗑</button>
                      </div>
                    </div>
                    <p class="qi-content">{{ q.content }}</p>
                    <div v-if="q.replies?.length" class="qi-replies">
                      <div v-for="r in q.replies" :key="r.id" class="qi-reply" :class="{official:r.official}">
                        <div class="uav" style="width:22px;height:22px;font-size:.62rem;background:linear-gradient(135deg,#059669,#10b981)">
                          {{ (r.author||'?')[0].toUpperCase() }}
                        </div>
                        <div class="qi-rb">
                          <div class="qi-rmeta">
                            <span class="qi-author">{{ r.author }}</span>
                    ...
                    <div class="qi-reply-form">
                      <textarea v-if="qaAdminReplyOpen[q.id]" v-model="qaAdminReplyText[q.id]"
                                class="qi-input" rows="2" placeholder="Trả lời..."
                                :ref="el=>el&&el.focus()"></textarea>
                      <div class="qi-rf-acts">
                        <button v-if="!qaAdminReplyOpen[q.id]" @click="qaAdminReplyOpen[q.id]=true"
                                class="btn btn-accent btn-sm">✏️ Trả lời</button>
                        <template v-else>
                          <button class="btn btn-ghost btn-sm" @click="qaAdminReplyOpen[q.id]=false">Hủy</button>
                          <button class="btn btn-accent btn-sm" @click="submitAdminQAReply(q)"
                                  :disabled="!qaAdminReplyText[q.id]?.trim()||qaAdminReplying[q.id]">
                            {{ qaAdminReplying[q.id]?'…':'Gửi' }}
                          </button>
                        </template>
                      </div>
                    </div>
              ...
              <!-- ════ PAYMENTS TAB — nằm TRONG panel-body ════ -->
              <div v-show="activeTab==='payments'" class="tab-inner">
                <!-- Stats row -->
                <div class="mini-stats" style="margin-bottom:1rem">
                  <div class="ms-it">
                    <span class="ms-n">{{ paymentStats.total }}</span>
                    <span class="ms-l">Tổng GD</span>
                  </div>
                  <div class="ms-it">
                    <span class="ms-n" style="color:var(--green)">{{ paymentStats.success }}</span>
                    <span class="ms-l">Thành công</span>
                  </div>
                  <div class="ms-it">
                    <span class="ms-n" style="color:var(--accent)">
                      {{ new Intl.NumberFormat('vi-VN',{style:'currency',currency:'VND'}).format(paymentStats.revenue) }}
                    </span>
                    <span class="ms-l">Doanh thu</span>
                  </div>
                </div>

                <!-- Filter -->
                <div class="filter-bar" style="margin-bottom:.85rem">
                  <button v-for="s in [{v:'ALL',l:'Tất cả'},{v:'SUCCESS',l:'✅ Thành công'},{v:'PENDING',l:'⏳ Chờ'},{v:'FAILED',l:'❌ Thất bại'}]"
                          :key="s.v"
                          :class="['ab','ab-filter',payStatusFilter===s.v?'ab-filter-active':'']"
                          @click="payStatusFilter=s.v; payPage=1">{{ s.l }}</button>
                </div>

                <div v-if="loadingPayments" class="center-state"><div class="spinner"></div></div>
                <div v-else-if="!filteredPayments.length" class="center-state">
                  <div class="empty-ico">💰</div>
                ...
                <template v-else>
                  <div class="tbl-wrap">
                    <table class="tbl">
                      ...
                      <tbody>
                      <tr v-for="(p, i) in pagedPayments" :key="p.id">
                        <td class="id-col">{{ (payPage-1)*PAY_PER_PAGE + i+1 }}</td>
                        <td>
                          <div class="ucell">
                            <div class="uav">{{ (p.username||'?')[0].toUpperCase() }}</div>
                            <span class="uname">{{ p.username }}</span>
                          </div>
                        </td>
                        <td>
                          <div class="ct">{{ p.courseTitle }}</div>
                        </td>
                        <td style="font-weight:700;color:var(--accent)">
                          {{ new Intl.NumberFormat('vi-VN',{style:'currency',currency:'VND'}).format(p.amount) }}
                        </td>
                        <td>
                          <span :class="['badge', p.status==='SUCCESS'?'badge-green':p.status==='PENDING'?'badge-yellow':'badge-red']">
                            {{ p.status==='SUCCESS'?'✅ Thành công':p.status==='PENDING'?'⏳ Chờ':'❌ Thất bại' }}
                          </span>
                        </td>
                        <td class="muted-col" style="font-family:monospace;font-size:.72rem">{{ p.txnRef }}</td>
                        <td class="muted-col">
                  ...
                  <div class="pay-footer">
                    <span class="pay-summary">
                      {{ filteredPayments.length }} giao dịch &nbsp;·&nbsp; Trang {{ payPage }}/{{ payTotalPages }}
                    </span>
                    <div v-if="payTotalPages > 1" class="tbl-pagination" style="margin:0">
                      <button class="pg-btn" :disabled="payPage===1" @click="payPage--">‹</button>
                      <button v-for="pg in payPageNumbers" :key="pg"
                              :class="['pg-btn', { active: pg===payPage, ellipsis: pg==='...' }]"
                              :disabled="pg==='...'"
                              @click="pg !== '...' && (payPage=pg)">{{ pg }}</button>
                      <button class="pg-btn" :disabled="payPage===payTotalPages" @click="payPage++">›</button>
                    </div>
                  </div>
                </template>
              </div>
        ...
        <div v-if="showReject" class="modal-overlay" @click.self="showReject=false">
          <div class="modal">
            <h2>❌ Từ chối đơn Giảng viên</h2>
            <p style="color:var(--muted);margin:.3rem 0 1rem">Người nộp: <strong>{{ rejectTarget?.fullName || rejectTarget?.username }}</strong></p>
            <div class="form-group">
              <label>Lý do từ chối <span style="color:var(--muted);font-weight:400;font-size:.8rem">(sẽ gửi thông báo cho user)</span></label>
              <textarea v-model="rejectNote" rows="3" placeholder="VD: Hồ sơ chưa đầy đủ, thiếu kinh nghiệm thực tế..."></textarea>
            </div>
            <div class="modal-actions">
              <button class="btn btn-ghost" @click="showReject=false">Hủy</button>
              <button class="btn" style="background:#dc2626;color:#fff;border:none" @click="confirmReject">Xác nhận từ chối</button>
            </div>
          </div>
        </div>
            ...
            <form @submit.prevent="saveCourse">
              ...
              <div class="form-group">
                <label style="display:flex;align-items:center;gap:.6rem;cursor:pointer;font-weight:600">
                  <input type="checkbox" v-model="cf.isFree" style="width:15px;height:15px;cursor:pointer;accent-color:var(--accent)"/>
                  Khóa học miễn phí
                </label>
              </div>
              <div v-if="!cf.isFree" class="form-group">
                <label>Học phí (VNĐ) *</label>
                <input v-model.number="cf.price" type="number" min="1000" step="1000" placeholder="VD: 299000"/>
                <div class="field-hint">VD: 299000 = 299.000 đ</div>
              </div>
              <div class="modal-actions">
...
const tabs = [
  ...
  { key:'applications',  icon:'📝', label:'Đơn GV' },
...
const payments=ref([]); const loadingPayments=ref(false)
const payStatusFilter=ref('ALL')
const payPage=ref(1); const PAY_PER_PAGE=10
...
const apps=ref([])
const adminQAList      = ref([])
const loadingAdminQA   = ref(false)
const qaAdminFilter    = ref(false)
const qaAdminReplyOpen = reactive({})
const qaAdminReplyText = reactive({})
const qaAdminReplying  = reactive({})
const loadingApps=ref(false); const appFilter=ref('PENDING')
const showReject=ref(false); const rejectTarget=ref(null); const rejectNote=ref('')
const editL=ref(null)
...
async function loadApps() {
  ...
  try {
    const url = appFilter.value
        ? `/admin/instructor-applications?status=${appFilter.value}`
        : '/admin/instructor-applications'
    apps.value = (await api.get(url)).data
  } catch(e) { showToast('Lỗi tải danh sách đơn!','error') }
  finally { loadingApps.value = false }
}

async function approveApp(app) {
  if (!confirm(`Duyệt đơn của ${app.fullName || app.username}?\nHọ sẽ được nâng lên INSTRUCTOR ngay lập tức.`)) return
  try {
    await api.post(`/admin/instructor-applications/${app.id}/approve`)
    app.status = 'APPROVED'
    app.reviewedAt = new Date().toISOString()
    showToast(`✅ Đã duyệt — ${app.fullName || app.username} trở thành Giảng viên!`)
  } catch(e) { showToast(e.response?.data?.error || 'Thao tác thất bại!', 'error') }
}
...
async function confirmReject() {
  ...
  try {
    await api.post(`/admin/instructor-applications/${rejectTarget.value.id}/reject`,
        { adminNote: rejectNote.value })
    rejectTarget.value.status = 'REJECTED'
    rejectTarget.value.adminNote = rejectNote.value
    rejectTarget.value.reviewedAt = new Date().toISOString()
...
async function loadAdminQA() {
  loadingAdminQA.value = true
  try {
    const r = await api.get(`/admin/questions?resolved=${qaAdminFilter.value}`)
    adminQAList.value = Array.isArray(r.data) ? r.data : []
  } catch { showToast('Lỗi tải câu hỏi!','error') }
  finally { loadingAdminQA.value = false }
}

async function toggleAdminQAResolve(q) {
  try {
    const r = await api.patch(`/questions/${q.id}/resolve`)
    q.resolved = r.data.resolved
    if (!qaAdminFilter.value && q.resolved)
      setTimeout(() => { adminQAList.value = adminQAList.value.filter(x=>x.id!==q.id) }, 600)
  } catch { showToast('Lỗi!','error') }
}

async function delAdminQA(qId) {
  if (!confirm('Xóa câu hỏi này?')) return
  try {
    await api.delete(`/questions/${qId}`)
    adminQAList.value = adminQAList.value.filter(q=>q.id!==qId)
    showToast('Đã xóa')
  } catch(e) { showToast(e.response?.data?.error||'Lỗi!','error') }
}

async function submitAdminQAReply(q) {
  const content = qaAdminReplyText[q.id]?.trim()
  if (!content) return
  qaAdminReplying[q.id] = true
  try {
    ...
    qaAdminReplyOpen[q.id] = false; qaAdminReplyText[q.id] = ''
    showToast('Đã trả lời!')
    if (!qaAdminFilter.value)
      setTimeout(() => { adminQAList.value = adminQAList.value.filter(x=>x.id!==q.id) }, 800)
  } catch(e) { showToast(e.response?.data?.error||'Lỗi!','error') }
  finally { qaAdminReplying[q.id] = false }
}

function switchTab(key) {
  activeTab.value = key
  if (key==='payments') loadPayments()
  if (key==='qa') loadAdminQA()
...
async function loadPayments() {
  ...
  try {
    const r = await api.get('/payment/admin/all')
    payments.value = Array.isArray(r.data) ? r.data : []
  } catch { showToast('Lỗi tải giao dịch!', 'error') }
  finally { loadingPayments.value = false }
}

const filteredPayments = computed(() => {
  if (payStatusFilter.value === 'ALL') return payments.value
  return payments.value.filter(p => p.status === payStatusFilter.value)
})

const payTotalPages = computed(() =>
    Math.max(1, Math.ceil(filteredPayments.value.length / PAY_PER_PAGE))
)

const pagedPayments = computed(() => {
  const start = (payPage.value - 1) * PAY_PER_PAGE
  return filteredPayments.value.slice(start, start + PAY_PER_PAGE)
})

const payPageNumbers = computed(() => {
  const total = payTotalPages.value
  const cur   = payPage.value
  if (total <= 7) return Array.from({length: total}, (_, i) => i+1)
  const pages = [1]
  if (cur > 3) pages.push('...')
  for (let i = Math.max(2, cur-1); i <= Math.min(total-1, cur+1); i++) pages.push(i)
...
const paymentStats = computed(() => {
  ...
  const revenue = success.reduce((s, p) => s + (p.amount || 0), 0)
  return { total: all.length, success: success.length, revenue }
})

// Reset trang khi đổi filter
watch(payStatusFilter, () => { payPage.value = 1 })
...
function openCourseModal(c=null) {
  editC.value=c
  cf.value=c?{title:c.title,description:c.description||'',instructor:c.instructor||'',credits:c.credits,status:c.status,price:c.price||0,isFree:c.isFree!==false}
      :{title:'',description:'',instructor:'',credits:3,status:'ACTIVE',price:0,isFree:true}
  showCM.value=true
}
...
async function recalcGrade(e) {
  ...
  try {
    const r = await api.post(`/grades/calculate/${e.id}`)
    if (!e.grade) e.grade = {}
    e.grade.score    = r.data.score
    e.grade.feedback = r.data.feedback
    showToast(`✅ ${e.user?.username}: ${r.data.score?.toFixed(1)} điểm`)
  } catch(err) {
...
async function recalcAll() {
  ...
  if (!confirm(`Tính lại điểm quiz cho tất cả ${enrollments.value.length} học viên?`)) return
  ...
  showToast(`✅ Đã tính ${ok} điểm${fail ? ` (${fail} không có quiz hoặc lỗi)` : ''}`)
...
async function loadQuizzes() {
  ...
  } catch(e) { console.error('Lỗi tải quiz:', e) }
...
async function loadCerts() {
  ...
  } catch { showToast('Tải chứng chỉ thất bại!', 'error') }
...
async function revokeCert(c) {
  ...
  } catch { showToast('Thu hồi thất bại!', 'error') }
}
...
async function loadNotifs(page=0) {
  ...
  try {
    ...
    console.error(e)
    allNotifs.value = []; notifTotalPages.value = 0; notifTotal.value = 0
  } finally { loadingNotifs.value = false }
}

async function clearAllNotifs() {
  if (!confirm(`Xóa tất cả ${notifTotal.value} thông báo? Không thể hoàn tác!`)) return
  try { await api.delete('/notifications/admin/clear-all'); showToast('Đã xóa tất cả thông báo!'); loadNotifs() }
  catch { showToast('Xóa thất bại!','error') }
}
...
onUnmounted(() => { Object.values(chartInst).forEach(ch=>{try{ch.destroy()}catch{}}); chartInst={} })
...
.pay-footer { display:flex; align-items:center; justify-content:space-between; margin-top:.75rem; flex-wrap:wrap; gap:.5rem; }
.pay-summary { font-size:.74rem; color:var(--muted); }

/* Pagination */
.tbl-pagination { display:flex; align-items:center; gap:.3rem; margin-top:.8rem; flex-wrap:wrap; }
.tbl-pagination .pg-info { margin-left:auto; font-size:.74rem; color:var(--muted); }
.pg-btn { min-width:30px; height:30px; padding:0 .35rem; border-radius:7px; border:1.5px solid var(--border); background:var(--surface); color:var(--text2); font-size:.76rem; font-weight:600; cursor:pointer; font-family:'Plus Jakarta Sans',sans-serif; transition:all .15s; display:flex; align-items:center; justify-content:center; }
...
.notif-scroll-wrap { background:var(--surface); border:1.5px solid var(--border); border-radius:12px; overflow-y:auto; max-height:520px; scrollbar-width:thin; scrollbar-color:var(--border) transparent; }
.notif-scroll-wrap::-webkit-scrollbar { width:5px; }
.notif-scroll-wrap::-webkit-scrollbar-thumb { background:var(--border); border-radius:100px; }
...
/* Applications */
.app-filter-bar { display:flex; gap:.4rem; flex-wrap:wrap; margin-bottom:.8rem; align-items:center; }
.ab-filter { background:var(--surface2); color:var(--muted); border:1.5px solid var(--border); font-size:.72rem; padding:.28rem .65rem; border-radius:7px; cursor:pointer; font-weight:600; transition:all .15s; font-family:'Plus Jakarta Sans',sans-serif; }
.ab-filter:hover { border-color:var(--accent); color:var(--accent); }
.ab-filter-active { background:var(--accent) !important; color:#fff !important; border-color:var(--accent) !important; }
.ab-yellow.ab-filter-active { background:#d97706 !important; border-color:#d97706 !important; }
.ab-green2.ab-filter-active { background:#16a34a !important; border-color:#16a34a !important; }
.ab-red2.ab-filter-active   { background:#dc2626 !important; border-color:#dc2626 !important; }
.app-cards-wrap { display:flex; flex-direction:column; gap:.7rem; }
.app-card { background:var(--surface); border:1.5px solid var(--border); border-radius:12px; overflow:hidden; }
.app-card-head { display:flex; align-items:flex-start; justify-content:space-between; padding:.85rem 1rem .6rem; gap:.5rem; }
.app-card-body { padding:0 1rem .7rem; display:flex; flex-direction:column; gap:.35rem; }
.app-field { display:flex; gap:.5rem; align-items:flex-start; font-size:.8rem; }
.app-label { color:var(--muted); font-weight:600; min-width:80px; flex-shrink:0; font-size:.73rem; padding-top:.1rem; }
.app-card-actions { display:flex; gap:.5rem; padding:.7rem 1rem; border-top:1px solid var(--border); background:var(--surface2); }
.app-link  { color:var(--accent); font-size:.8rem; font-weight:600; text-decoration:none; }
.app-link:hover { text-decoration:underline; }
.muted { color:var(--muted); font-size:.82rem; }

/* cert */
.cert-code-link { font-size:.78rem; font-family:monospace; color:var(--accent); text-decoration:none; }
.cert-code-link:hover { text-decoration:underline; }

/* Q&A */
.qa-inbox { display:flex; flex-direction:column; gap:.7rem; }
.qi-card { background:var(--surface2); border:1.5px solid var(--border); border-radius:11px; padding:.9rem; transition:border-color .15s; }
.qi-card:hover { border-color:var(--accent); }
.qi-card.resolved { border-color:#a7f3d0; background:#f0fdf4; opacity:.85; }
.qi-head { display:flex; align-items:flex-start; gap:.55rem; margin-bottom:.55rem; flex-wrap:wrap; }
.qi-meta { flex:1; display:flex; flex-direction:column; gap:.08rem; }
.qi-author { font-size:.8rem; font-weight:700; }
.qi-context { font-size:.7rem; color:var(--muted); }
.qi-time { font-size:.66rem; color:var(--muted); }
.qi-acts { display:flex; gap:.28rem; align-items:center; margin-left:auto; }
.qi-content { font-size:.82rem; color:var(--text); line-height:1.6; margin:0 0 .65rem; white-space:pre-wrap; }
...
.qi-reply.official { background:#f0fdf4; border-radius:6px; padding:.35rem; }
.qi-rb { flex:1; }
.qi-rmeta { display:flex; align-items:center; gap:.3rem; margin-bottom:.15rem; flex-wrap:wrap; }
.qi-rtext { font-size:.79rem; color:var(--text); line-height:1.5; margin:0; white-space:pre-wrap; }
.qi-reply-form { margin-top:.3rem; }
.qi-input { width:100%; padding:.45rem .7rem; background:var(--surface); border:1.5px solid var(--border); border-radius:7px; color:var(--text); font-size:.81rem; font-family:inherit; resize:none; outline:none; transition:border-color .15s; box-sizing:border-box; display:block; margin-bottom:.35rem; }
.qi-input:focus { border-color:var(--accent); box-shadow:0 0 0 3px var(--accent-light); }
.qi-rf-acts { display:flex; gap:.35rem; }
.ab-active { background:var(--accent) !important; color:#fff !important; border-color:var(--accent) !important; }

/* Sidebar toggle */
.sidebar-toggle { display:none; background:none; border:none; font-size:1.2rem; cursor:pointer; color:var(--text); padding:.2rem .4rem; border-radius:6px; line-height:1; }
.sidebar-toggle:hover { background:var(--surface2); }

/* ─── RESPONSIVE ─── */
@media (max-width: 768px) {
  .page-header { padding: .8rem 1rem .6rem; }
  .page-header h1 { font-size: 1.2rem; }
  .admin-shell { flex-direction: column; position: relative; }
  .sidebar {
    position: fixed; top: 0; left: 0;
    width: 220px; height: 100dvh; z-index: 300;
  ...
  .panel { width: 100%; }
  .panel-hd { padding: 0 .9rem; gap: .6rem; }
  .panel-body { padding: .8rem .9rem; }
  .phd-title { font-size: .88rem; }
  .kpi-row { grid-template-columns: repeat(2, 1fr) !important; }
  .chart-row { grid-template-columns: 1fr !important; }
  .tab-inner > table { min-width: 600px; }
  .modal { width: 100% !important; max-width: 100% !important; border-radius: 16px 16px 0 0 !important; margin-top: auto; max-height: 92dvh; overflow-y: auto; }
  .modal-overlay { align-items: flex-end !important; padding: 0 !important; }
}

@media (max-width: 480px) {
  .kpi-row { grid-template-columns: 1fr 1fr !important; }
  .kpi-card { padding: .65rem .7rem; }
  .panel-hd { height: auto; min-height: 52px; padding: .5rem .9rem; flex-wrap: wrap; }
  .hd-right { flex-wrap: wrap; gap: .4rem; }
  .btn-sm { font-size: .7rem; padding: .28rem .6rem; }
